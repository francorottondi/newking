...
while validacion==False:
    ...
    login.title("Login")
    login.resizable(0,0)
    login.geometry("+550+300")

    def loguear():
        global validacion
        attemptLogin = userEntry.get()
        attemptPass = passEntry.get()
        if validacion_usuario(attemptLogin,attemptPass):
            validacion=True
            login.destroy()
        else:
            pass
    ...
    def validacion_usuario(attemptLogin, attemptPass):
        if attemptLogin.lower() =="franco" and attemptPass=="1234":
            messagebox.showinfo("Login",f"Bienvenido {attemptLogin}.")
            return True
        else:
            messagebox.showerror("Login","Usuario/Contraseña Incorrecto/s")
    ...
    framelog=Frame(login)
    framelog.grid(row=0, column=0,padx=5,pady=5)

    usuariosRegistrados=(("Franco", 1234),("Mica",1234))

    #Labels
    userLabel=Label(framelog,text="User: ")
    userLabel.grid(row=0,column=0,padx=5,sticky=E)

    passLabel=Label(framelog,text="Password: ")
    passLabel.grid(row=1,column=0,padx=5,sticky=E)
    #Entrys
    userEntry=Entry(framelog,bd=2)
    userEntry.grid(row=0,column=1)
    userEntry.focus()
    passEntry=Entry(framelog,bd=2,show="*")
    passEntry.grid(row=1,column=1)
    #BotonLogin
    loginbut=Button(framelog,text="Login",bd=1,command=loguear)
    loginbut.grid(row=3,column=1,columnspan=1,padx=2,sticky=W)
    # BotonCerrar
    cerrBut = Button(framelog, text="Cerrar", bd=1, command=cerrar)
    cerrBut.grid(row=3, column=0,columnspan=1,padx=2,sticky=E)
...
if validacion==False:
    ...
    def edit_products():
        ...
        ventanaedicion.resizable(0,0)
        
        # Nombre viejo
        Label(ventanaedicion, text='Antiguo Nombre: ').grid(row=0, column=1)
        Entry(ventanaedicion, textvariable=StringVar(ventanaedicion, value=name), state='readonly').grid(row=0,
                                                                                                         column=2)
        # Nuevo Nombre
        Label(ventanaedicion, text='Nuevo Nombre: ').grid(row=1, column=1)
        nombre_nuevo = Entry(ventanaedicion,textvariable=StringVar(ventanaedicion, value=name))
        nombre_nuevo.grid(row=1, column=2)
        nombre_nuevo.focus()
        # Precio antiguo
        Label(ventanaedicion, text='Precio Antiguo: ').grid(row=2, column=1)
        Entry(ventanaedicion, textvariable=StringVar(ventanaedicion, value=precio_viejo), state='readonly').grid(row=2,
                                                                                                                 column=2)
        # Precio Nuevo
        Label(ventanaedicion, text='Precio Nuevo: ').grid(row=3, column=1)
        precio_nuevo = Entry(ventanaedicion,textvariable=StringVar(ventanaedicion, value=precio_viejo))
        precio_nuevo.grid(row=3, column=2)
        # Rubro viejo
        Label(ventanaedicion, text='Antiguo Rubro: ').grid(row=4, column=1)
        Entry(ventanaedicion, textvariable=StringVar(ventanaedicion, value=rubro_viejo), state='readonly').grid(row=4,
                                                                                                                column=2)
        # Rubro Nombre
        Label(ventanaedicion, text='Nuevo Rubro: ').grid(row=5, column=1)
        rubro_nuevo = Entry(ventanaedicion,textvariable=StringVar(ventanaedicion, value=rubro_viejo))
        rubro_nuevo.grid(row=5, column=2)

        # Boton para aceptar cambios
        Button(ventanaedicion, text="Guardar Cambios",
               command=lambda: guardar_cambios(nombre_nuevo.get(), name, precio_nuevo.get(), precio_viejo,
                                               rubro_nuevo.get(), rubro_viejo)).grid(row=6, column=2, sticky=W)
